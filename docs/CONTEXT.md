...
| [ANI_MAKER_SPEC.md](ANI_MAKER_SPEC.md) | Design spec for the `animaker` tool | **Ghost, not live, as of 2026-09-18** — rewritten as a v2 spec (a full replacement, not an extension) describing a multi-part rig/props model that `animaker/pkg/editor` does not implement yet. Current code still implements the v1 flipbook model the spec's own "History" section describes as superseded. Don't read this file as describing current `animaker/` behavior at all until the rewrite lands — update this row when it does. |
